...
import { Task } from './page'; // Asegúrate de que la importación sea correcta según tu estructura de archivos
...
const WeeklyCalendar = ({ tasks }: { tasks: Task[] }) => {
  ...
  return (
    <TransformWrapper initialScale={1} minScale={0.1} maxScale={4} limitToBounds={true} wheel={{ step: 0.2 }}>
      <TransformComponent>
        <div className="overflow-x-auto">
          <div className="flex justify-start mb-4">
            <input type="date" value={format(selectedStartDate, 'yyyy-MM-dd')} onChange={(e) => setSelectedStartDate(parseISO(e.target.value))} className="mr-2" />
            <input type="date" value={format(selectedEndDate, 'yyyy-MM-dd')} onChange={(e) => setSelectedEndDate(parseISO(e.target.value))} />
          </div>
            ...
            {tasks.filter(task => task.startDate && isWithinInterval(parseISO(task.startDate), { start: selectedStartDate, end: selectedEndDate }))
              ...
              .flatMap((task, index) => [
                ...(task.subTasks?.map((subTask, subIndex) => {
                  const left = daysInRange.findIndex(day => format(day, 'yyyy-MM-dd') === format(parseISO(subTask.startDate), 'yyyy-MM-dd')) * 30 + 5;
                  const top = (index * 30 + subIndex * 30) + 40; // Ajusta el espacio vertical entre tareas
                  const width = subTask.durationInDays * 30;
                  return (
                    <div key={subTask.id} className="absolute" style={{ left: `${left}px`, top: `${top}px`, width: `${width}px` }}>
                      <div className="bg-gray-200 h-4 rounded-full overflow-hidden">
                        <div className="bg-blue-600 h-4 rounded-full" style={{ width: `${subTask.progress}%` }}></div>
                      </div>
                      <span className="absolute w-full text-center text-xs" style={{ left: '0px' }}>
